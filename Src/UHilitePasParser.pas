...
type
  ...
  private
    fOnElement: TParseElementEvent;   // OnElement event handler
    fOnLineBegin: TParseLineEvent;    // OnLineBegin event handler
    fOnLineEnd: TParseLineEvent;      // OnLineEnd event handler
  protected
  ...
  public
    procedure Parse(const SrcStm: TStream);
      ...
      token is parsed.
        @param SrcStm [in] tream containing Pascal source.
      }
    ...
    property OnLineBegin: TParseLineEvent
      ...
      {Event triggered just before first element of a new line is parsed. Users
...
begin
  ...
  try
    // Intialise: not in assembler and line not started
    InASM := False;
    BetweenLines := True;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Elem := heAssembler;
          case Lexer.Token of
            // we end assembler when an "end" keyword is encounterede
            tkKeyword:
            begin
            ...
            // handle tokens that are not formatted as assembler (note we differ
            // from Delphi editor here in that we don't format strings specially
            // in assembler - they take on attributes of assembler
        ...
        // Emit the element: if we're starting a line, notify of that first
